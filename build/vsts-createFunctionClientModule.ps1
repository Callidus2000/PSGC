...
$parentModule = 'PSGC'
if (-not $ModuleName) { $ModuleName = 'PSGC.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\PSGC'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\PSGC.psd1"
$prereqHash = @{
